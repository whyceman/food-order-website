    ...
    return (
        <div className="w-[95%] mx-auto px-4 py-12">
            ...
            <div className="grid grid-cols-2 gap-6 md:grid-cols-4 py-6 ">
                {categories.map((item) => (
                    <div key={item.id} className="bg-gray-100 rounded-lg flex md:justify-between gap-2 items-center px-2 py-4">
                        <h2 className="font-bold sm:text-xl">{item.name}</h2>
                        <img src={item.image} alt={item.name} className='w-10 md:w-20' />
                    </div>
                ))}
            </div>
        </div>
